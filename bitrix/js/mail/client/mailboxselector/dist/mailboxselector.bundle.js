...
(function (exports, main_core) {
	...
	class MailboxSelector {
		...
		#renderTitleButton(root, selectorConfig) {
			...
			const titleText = this.isAllMailMode ? main_core.Loc.getMessage('MAIL_CLIENT_ALL_MAIL') : selectorConfig.titleText || '';
			const hoverText = this.isAllMailMode ? main_core.Loc.getMessage('MAIL_CLIENT_ALL_MAIL') : selectorConfig.titleHoverText || '';
		...
		#renderAllMailItem(config) {
			...
			<span class="mail-menu-popup-item-text-wrapper">
				<span class="main-buttons-item-text">${main_core.Loc.getMessage('MAIL_CLIENT_ALL_MAIL')}</span>
			</span>
		`;
			const counter = main_core.Tag.render`<span class="main-buttons-item-counter ${counterClass}">${count}</span>`;
			return {
		...
		`;
			if (mailbox.isLocked) {
				main_core.Dom.append(main_core.Tag.render`<span class="mail-connect-lock-icon"></span>`, wrapper);
			}
			const counter = main_core.Tag.render`<span class="main-buttons-item-counter ${counterClass}">${count}</span>`;
			return {
				html: wrapper.outerHTML + ' ' + counter.outerHTML,
		...
		#renderAddMailboxItem(addMailbox) {
			...
			if (addMailbox.isLocked) {
				const lockNode = main_core.Tag.render`
				<div id="mail-connect-mailbox-add-lock-item">
					<span class="mail-connect-lock-text">${main_core.Loc.getMessage('MAIL_CLIENT_MAILBOX_ADD')}</span>
			...
			const textNode = main_core.Tag.render`<span class="main-buttons-item-text">${main_core.Loc.getMessage('MAIL_CLIENT_MAILBOX_ADD')}</span>`;
			return {
				text: main_core.Loc.getMessage('MAIL_CLIENT_MAILBOX_ADD'),
		...
		onMailboxMenuClick() {
			const popup = BX.Main.MenuManager.create(this.mailboxPopupMenuId, this.mailboxMenuToggle, this.mailboxMenu, {
				events: {
					onPopupFirstShow: () => {
			...
			if (menuContainer) {
				main_core.Dom.attr(menuContainer, 'data-test-id', 'mail_sync-panel__mailbox-menu');
			}
			popup.popupWindow.isShown() ? popup.close() : popup.show();
		}
		getCurrentMailboxId() {
			return this.mailboxMenuToggle && this.mailboxMenuToggle.dataset && this.mailboxMenuToggle.dataset.mailboxId ? this.mailboxMenuToggle.dataset.mailboxId : null;
		}
		...
		updateAllMailBadge(count) {
			...
			this.mailboxMenu[this.allMailMenuIndex] = this.setMailboxTitleMenuUnseenCounter(this.mailboxMenu[this.allMailMenuIndex], count);
			BX.Main.MenuManager.destroy(this.mailboxPopupMenuId);
		}
		setMailboxTitleMenuUnseenCounter(mailboxMenuItem, count) {
			...
			const counterClass = className ? `main-buttons-item-counter ${className}` : 'main-buttons-item-counter';
			const counterHtml = main_core.Tag.render`<span class="${counterClass}">${count}</span>`.outerHTML;
		...
		syncTopLevelCounter() {
			const unseen = BX.Mail.Home && BX.Mail.Home.mailboxCounters ? BX.Mail.Home.mailboxCounters.getTotalCounter() : 0;
			if (typeof top.B24 === 'object' && typeof top.B24.updateCounters === 'function') {
				top.B24.updateCounters({
					mail_unseen: unseen
				});
			}
	...
	exports.MailboxSelector = MailboxSelector;

})(this.BX.Mail.Client = this.BX.Mail.Client || {}, BX);
//# sourceMappingURL=mailboxselector.bundle.js.map
