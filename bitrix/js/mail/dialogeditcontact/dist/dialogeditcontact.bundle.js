...
(function (exports, main_core, mail_sidepanelwrapper, ui_dialogs_messagebox, mail_avatar, main_core_events) {
	...
	class DialogEditContact {
		...
		static saveContact(name, email, id = 'new') {
			const data = mail_avatar.Avatar.getAvatarData({
				fullName: name,
				email: email
			});
			let contactData = {
		...
		static showErrorEmailAlreadyExists(responseError, errorAlreadyExistLink, emailContainer) {
		...
		static openDialog(titleText, contactConfig) {
			...
			const emailInput = main_core.Tag.render`<input data-role="input-field" type="text" class="ui-ctl-element" value="" placeholder="info@example.com"  ${disablingEmailInputAttribute}>`;
			const emailInputWrapper = main_core.Tag.render`<div class="ui-ctl ui-ctl-textbox ui-ctl-w100 ${disablingEmailInputClass}">
			${emailInput}
		</div>`;
			emailInput.value = currentEmail;
			const errorTitleEmailIsIncorrect = main_core.Tag.render`<div class="ui-alert ui-alert-danger mail-addressbook-error-box">
			<span class="ui-alert-message">${main_core.Loc.getMessage("MAIL_DIALOG_EDIT_CONTACT_EMAIL_ERROR")}</span>
		</div>`;
			const errorTitleEmailIsAlreadyExists = main_core.Tag.render`<div class="ui-alert ui-alert-danger mail-addressbook-error-box">
			<span class="ui-alert-message">${main_core.Loc.getMessage("MAIL_DIALOG_EDIT_CONTACT_EMAIL_ERROR_EMAIL_IS_ALREADY_EXISTS")}</span>
			<br>
		</div>`;
			const errorAlreadyExistLink = errorTitleEmailIsAlreadyExists.querySelector('[data-role="contact-email"]');
			const emailContainer = main_core.Tag.render`<div data-role="email-container" class="mail-addressbook-dialogeditcontact-item">
			<label class="mail-addressbook-dialogeditcontact-lable">${main_core.Loc.getMessage("MAIL_DIALOG_EDIT_CONTACT_EMAIL_TITLE")}
			...
			const nameInput = main_core.Tag.render`<input data-role="input-field" type="text" class="ui-ctl-element" value="" placeholder="">`;
			const nameInputWrapper = main_core.Tag.render`<div class="ui-ctl ui-ctl-textbox ui-ctl-w100">
			${nameInput}
		</div>`;
			nameInput.value = currentName;
			const nameItem = main_core.Tag.render`<div data-role="name-container" class="mail-addressbook-dialogeditcontact-item">
			<label class="mail-addressbook-dialogeditcontact-lable">${main_core.Loc.getMessage("MAIL_DIALOG_EDIT_CONTACT_NAME_TITLE")}
				<div id="mail-addressbook-dialogeditcontact-contact-email-container" class="ui-ctl ui-ctl-textbox mail-addressbook-dialogeditcontact-field">
					${nameInputWrapper}
				</div>
			</label>
