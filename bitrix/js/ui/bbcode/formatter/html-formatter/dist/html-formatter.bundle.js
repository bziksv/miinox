...
(function (exports, main_core, ui_bbcode_formatter, ui_bbcode_model, ui_smiley, ui_codeParser, ui_videoService) {
	'use strict';
			...
			super({
				...
				after({
					...
					if (mode === 'void' || mode === 'collapsed') {
						const container = main_core.Tag.render`<div class="ui-typography-container --${mode}"></div>`;
		...
		constructor(options = {}) {
			super({
				...
				convert({
					...
					const splits = this.#smileyParser.parse(text);
					if (splits.length === 0) {
						return document.createTextNode(text);
					}
		...
		constructor(options = {}) {
			super({
				...
				after({
					...
					const container = main_core.Tag.render`<div class="ui-typography-table-scroll"></div>`;
					container.appendChild(element);
					return container;
				},
				...options
			...
			super({
				...
				convert({
					...
					<button type="button" 
						...
						title="${main_core.Loc.getMessage('HTML_FORMATTER_COPY_TO_CLIPBOARD')}"
						aria-label="${main_core.Loc.getMessage('HTML_FORMATTER_COPY_TO_CLIPBOARD')}"
					>
		...
		constructor(options) {
			super({
				...
				convert({
					...
					if (main_core.Type.isStringFilled(mentionSettings?.urlTemplate?.user)) {
		...
		constructor(options = {}) {
			super({
				...
				convert({
					...
					const mentionSettings = formatter.getMentionSettings();
					if (main_core.Type.isStringFilled(mentionSettings?.urlTemplate?.department)) {
		...
		constructor(options = {}) {
			super({
				...
				convert({
					...
					const mentionSettings = formatter.getMentionSettings();
					if (main_core.Type.isStringFilled(mentionSettings?.urlTemplate?.project)) {
			...
			super({
				...
				convert({
					...
					const info = data?.files?.find(file => {
						return file.serverFileId.toString() === serverFileId.toString() || `n${file.customData?.objectId.toString()}` === serverFileId.toString();
					});
					if (!info) {
						return createTextNode();
					}
...
})(this.BX.UI.BBCode.Formatter = this.BX.UI.BBCode.Formatter || {}, BX, BX.UI.BBCode, BX.UI.BBCode, BX.UI.Smiley, BX.UI.CodeParser, BX.UI.VideoService);
