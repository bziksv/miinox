...
(function (exports, calendar_util, main_core, calendar_controls, main_core_events, main_popup, calendar_roomsmanager, calendar_categorymanager, ui_infoHelper, ui_icons_b24, ui_datePicker, main_loader, ui_navigationpanel, calendar_entry, ui_buttons, ui_dialogs_messagebox, calendar_planner, ui_entitySelector, ui_avatar, intranet_controlButton, main_sidepanel) {
	'use strict';
	...
	class Reminder extends main_core_events.EventEmitter {
		...
		create(params) {
			...
			if (!this.viewMode) {
				this.DOM.addButton = this.DOM.wrap.appendChild(main_core.Tag.render`
				<span class="calendar-notification-add-btn-wrap">
					<span class="calendar-notification-text">${main_core.Loc.getMessage('EC_REMIND1_ADD')}</span>
		...
		addValue(value, emitChanges = true, changedByUser = true) {
			...
			if (main_core.Type.isPlainObject(value) && !this.selectedValues.includes(formattedValue)) {
				if (main_core.Type.isInteger(parseInt(value.before)) && main_core.Type.isInteger(parseInt(value.time))) {
					item = this.DOM.wrap.appendChild(main_core.Tag.render`
					...
					if (!this.viewMode) {
						item.appendChild(main_core.Tag.render`<span class="calendar-reminder-clear-icon" data-bxc-value="${formattedValue}"/>`);
					}
				} else if (value.type === 'date' && main_core.Type.isDate(value.value)) {
					item = this.DOM.wrap.appendChild(main_core.Tag.render`<span class="calendar-reminder-item">
					<span class="calendar-reminder-item-title">
						${calendar_util.Util.formatDateUsable(value.value) + ' ' + calendar_util.Util.formatTime(value.value)}
					</span>
				</span>`);
					if (!this.viewMode) {
						item.appendChild(main_core.Tag.render`<span class="calendar-reminder-clear-icon" data-bxc-value="${formattedValue}"/>`);
		...
		adjustCalendarSubmenuPopup(popup) {
			...
			if (main_core.Type.isDomNode(textNode)) {
				main_core.Dom.clean(textNode);
				const input = textNode.appendChild(main_core.Tag.render`<input id="inp-${Math.round(Math.random() * 100000)}" type="text" class="calendar-field calendar-field-datetime" value="" autocomplete="off" placeholder="${main_core.Loc.getMessage('EC_REMIND1_CUSTOM_PLACEHOLDER')}"/>`);
	...
	class Location {
		...
		create() {
			...
			this.DOM.inputWrap = this.DOM.wrapNode.appendChild(main_core.Tag.render`
			<div class="calendar-field-block"></div>
		`);
			this.DOM.alertIconLocation = main_core.Tag.render`
			<div class="ui-alert-icon-danger calendar-location-alert-icon" data-hint-no-icon="Y" data-hint="${main_core.Loc.getMessage('EC_LOCATION_OVERFLOW')}">
			<i></i>
			</div>
		`;
			if (this.inlineEditModeEnabled) {
				this.DOM.inlineEditLinkWrap = this.DOM.wrapNode.appendChild(main_core.Tag.render`
				<div class="calendar-field-place-link calendar-location-readonly">${this.DOM.inlineEditLink = main_core.Tag.render`
					<span class="calendar-text-link">${main_core.Loc.getMessage('EC_REMIND1_ADD')}</span>
				`}
				</div>
			`);
			...
			this.DOM.inputWrapInner = this.DOM.inputWrap.appendChild(main_core.Tag.render`
		...
		`);
			...
			if (this.hideLocationLock) {
				...
				this.DOM.lockIcon = main_core.Tag.render`
		...
		addLocationRemoveButton() {
			...
			if ((this.value.value || this.value.str || this.value.text) && !this.viewMode && !this.DOM.removeLocationButton && this.value.text !== '') {
				this.DOM.removeLocationButton = wrap.appendChild(main_core.Tag.render`
				<span class="calendar-location-clear-btn-wrap calendar-location-readonly">
					<span class="calendar-location-clear-btn-text">${main_core.Loc.getMessage('EC_LOCATION_CLEAR_INPUT')}</span>
				</span>
			`);
			...
			if (this.mode === this.LINE_MODE) {
				for (let i = 0; i < this.defaultColors.length; i++) {
					this.colors.push({
						...
						node: this.DOM.wrap.appendChild(main_core.Tag.render`
				...
				this.DOM.customColorNode = this.DOM.wrap.appendChild(main_core.Tag.render`
				<li class="calendar-field-colorpicker-color-item" style="background-color: transparent; width: 0">
					<span class="calendar-field-colorpicker-color"></span>
				</li>`);
				this.DOM.customColorLink = this.DOM.wrap.appendChild(main_core.Tag.render`
				<li class="calendar-field-colorpicker-color-item-more">
					<span class="calendar-field-colorpicker-color-item-more-link">${main_core.Loc.getMessage('EC_COLOR')}</span>
				</li>
			`);
				...
				this.DOM.colorIcon = this.DOM.wrap.appendChild(main_core.Tag.render`
	...
	class SectionSelector {
		...
		create() {
			if (this.mode === 'textselect') {
				...
				this.DOM.selectImageWrap = this.DOM.select.appendChild(main_core.Tag.render`<span class="calendar-field-choice-calendar-img"></span>`);
				this.DOM.selectInnerText = this.DOM.select.appendChild(main_core.Tag.render`<span class="calendar-field-choice-calendar-name">${main_core.Text.encode(main_core.Loc.getMessage('EC_CALENDAR_SECTION_TITLE') + ' ' + this.getCurrentTitle())}</span>`);
			} else if (this.mode === 'location') {
				this.DOM.select = this.DOM.outerWrap;
				this.DOM.selectImageWrap = this.DOM.select.appendChild(main_core.Tag.render`<span class="calendar-field-choice-calendar-img"></span>`);
				this.DOM.selectInnerText = this.DOM.select.appendChild(main_core.Tag.render`<span class="calendar-field-choice-calendar-name calendar-field-choice-calendar-name-location">${main_core.Text.encode(main_core.Loc.getMessage('EC_CALENDAR_LOCATION_TITLE') + ' ' + this.getCurrentTitle())}</span>`);
			} else if (this.mode === 'inline') {
				this.DOM.select = this.DOM.outerWrap;
			} else {
				this.DOM.select = this.DOM.outerWrap.appendChild(main_core.Tag.render`
				<div class="calendar-field calendar-field-select"></div>
			`);
				this.DOM.innerValue = this.DOM.select.appendChild(main_core.Tag.render`
				<div class="calendar-field-select-icon" style="background-color: ${this.getCurrentColor()}"></div>
			`);
				if (this.mode === 'full') {
					this.DOM.selectInnerText = this.DOM.select.appendChild(main_core.Tag.render`<span>${main_core.Text.encode(this.getCurrentTitle())}</span>`);
				}
			}
		}
		initEventHandlers() {
		...
		updateSectionImageNode(section) {
			...
			if (section?.type || this.defaultCalendarType) {
				...
				if (imageSrc) {
					imageNode = main_core.Tag.render`<img class="calendar-field-choice-calendar-img-value" src="${encodeURI(imageSrc)}" alt="">`;
				} else if (type === 'group') {
					imageNode = main_core.Tag.render`<div class="ui-icon ui-icon-common-user-group"><i></i></div>`;
				} else if (type === 'user') {
					imageNode = main_core.Tag.render`<div class="ui-icon ui-icon-common-user"><i></i></div>`;
				} else {
					imageNode = main_core.Tag.render`<div class="ui-icon ui-icon-common-bitrix24"><i></i></div>`;
				}
	...
	class RepeatSelector {
		...
		create() {
			main_core.Event.bind(this.DOM.rruleType, 'click', this.#showRruleTypeMenu);
			main_core.Event.bind(this.DOM.rruleCount, 'click', this.#showRruleCountMenu);
		...
		initDatePicker() {
			this.#untilPicker ??= new ui_datePicker.DatePicker({
		...
		#showRruleTypeMenu = () => {
			this.#rruleTypeMenu ??= new main_popup.Menu({
		...
		#showRruleCountMenu = () => {
			this.#rruleCountMenu ??= new main_popup.Menu({
	...
	class SelectInput {
		...
		#renderLoader() {
			...
			this.loaderContainer = main_core.Tag.render`<div style="position: absolute; inset: 0;"></div>`;
	...
	class ViewSelector extends main_core_events.EventEmitter {
		...
		create() {
			this.DOM.wrap = main_core.Tag.render`<div class="calendar-view-switcher-selector"></div>`;
			this.DOM.selectorText = main_core.Tag.render`<div class="calendar-view-switcher-text"></div>`;
			this.DOM.selectorTextInner = this.DOM.selectorText.appendChild(main_core.Tag.render`<div class="calendar-view-switcher-text-inner"></div>`);
			this.DOM.wrap.appendChild(this.DOM.selectorText);
			this.DOM.wrap.appendChild(main_core.Tag.render`<div class="calendar-view-switcher-dropdown"></div>`);
			main_core.Event.bind(this.DOM.wrap, 'click', this.showPopup.bind(this));
			this.DOM.viewModeTextInner = this.DOM.selectorText.appendChild(main_core.Tag.render`<div class="calendar-view-switcher-text-mode-inner" style="display: none;"></div>`);
			this.created = true;
		}
		getOuterWrap() {
			if (!this.created) {
	...
	class DateTimeControl extends main_core_events.EventEmitter {
		...
		create() {
			if (main_core.Type.isDomNode(this.DOM.outerWrap)) {
				...
				this.DOM.leftInnerWrap = this.DOM.outerWrap.appendChild(main_core.Tag.render`<div class="calendar-field-block calendar-field-block-left"></div>`);
				this.DOM.fromDate = this.DOM.leftInnerWrap.appendChild(main_core.Tag.render`
				<input class="calendar-field calendar-field-datetime" value="" type="text" autocomplete="off" style="width: ${this.DATE_INPUT_WIDTH}px;"/>
			`);
				if (this.inlineEditMode) {
					this.DOM.fromDateText = this.DOM.leftInnerWrap.appendChild(main_core.Tag.render`<span class="calendar-field-value calendar-field-value-date"></span>`);
				}
				this.DOM.fromTime = this.DOM.leftInnerWrap.appendChild(main_core.Tag.render`
			...
			`);
				if (this.inlineEditMode) {
					this.DOM.fromTimeText = this.DOM.leftInnerWrap.appendChild(main_core.Tag.render`<span class="calendar-field-value calendar-field-value-time"></span>`);
				}
				this.DOM.betweenSpacer = this.DOM.outerWrap.appendChild(main_core.Tag.render`<div class="calendar-field-block calendar-field-block-between" />`);
				this.DOM.rightInnerWrap = this.DOM.outerWrap.appendChild(main_core.Tag.render`<div class="calendar-field-block calendar-field-block-right"></div>`);
				this.DOM.toTime = this.DOM.rightInnerWrap.appendChild(main_core.Tag.render`
				<input class="calendar-field calendar-field-time" value="" type="text" autocomplete="off" style="width: ${this.TIME_INPUT_WIDTH}px; max-width: ${this.TIME_INPUT_WIDTH}px;"/>
			`);
				if (this.inlineEditMode) {
					this.DOM.toTimeText = this.DOM.rightInnerWrap.appendChild(main_core.Tag.render`<span class="calendar-field-value calendar-field-value-time"></span>`);
				}
				this.DOM.toDate = this.DOM.rightInnerWrap.appendChild(main_core.Tag.render`
				<input class="calendar-field calendar-field-datetime" value="" type="text" autocomplete="off" style="width: ${this.DATE_INPUT_WIDTH}px;"/>
			`);
				if (this.inlineEditMode) {
					this.DOM.toDateText = this.DOM.rightInnerWrap.appendChild(main_core.Tag.render`<span class="calendar-field-value calendar-field-value-date"></span>`);
				}
				...
				const fullDayWrap = this.DOM.outerWrap.appendChild(main_core.Tag.render`
				<span class="calendar-event-full-day"></span>
			`);
				this.DOM.fullDay = fullDayWrap.appendChild(main_core.Tag.render`
				<input value="Y" type="checkbox" id="{this.UID}"/>
			`);
				fullDayWrap.appendChild(main_core.Tag.render`<label for="{this.UID}">${main_core.Loc.getMessage('EC_ALL_DAY')}</label>`);
			}
		...
		initDatePicker() {
			this.#dateFromPicker ??= new ui_datePicker.DatePicker({
			...
			this.#dateToPicker ??= new ui_datePicker.DatePicker({
		...
		showTzFromMenu() {
			this.#tzFromMenu ??= new main_popup.Menu({
		...
		showTzToMenu() {
			this.#tzToMenu ??= new main_popup.Menu({
	...
	class UserPlannerSelector extends main_core_events.EventEmitter {
		...
		setValue(params) {
			...
			if (this.entryId && this.entry && this.entry.data.PARENT_ID && (this.entry.data.EVENT_TYPE === '#shared#' || this.entry.data.EVENT_TYPE === '#shared_crm#') && this.entry.getCurrentStatus() !== false) {
				...
				this.conferenceButton = main_core.Tag.render`
				<div class="calendar-text-link --gray">${main_core.Loc.getMessage('EC_CONFERENCE_START')}</div>
			`;
			...
			} else if (BX?.Intranet?.ControlButton && this.DOM.videocallWrap && this.entryId && this.entry.getCurrentStatus() !== false) {
				main_core.Dom.clean(this.DOM.videocallWrap);
				main_core.Dom.removeClass(this.DOM.videocallWrap, 'calendar-videocall-hidden');
				const calendarContext = calendar_util.Util.getCalendarContext();
				const items = calendarContext?.isCollabUser ? ['chat', 'videocall', 'task'] : ['chat', 'videocall', 'blog_post', 'task'];
		...
		static getUserAvatarNode(user) {
			...
			if (user.COLLAB_USER) {
				...
				imageNode = main_core.Tag.render`<div title="${main_core.Text.encode(user.DISPLAY_NAME)}" class="ui-icon ${defaultAvatarClass}"><i></i></div>`;
			} else {
				imageNode = main_core.Tag.render`
				<img
					title="${main_core.Text.encode(user.DISPLAY_NAME)}"
	...
	class EmailSelectorControl extends main_core_events.EventEmitter {
		...
		constructor(params) {
			...
			this.DOM.componentWrap = this.DOM.select.parentNode.appendChild(main_core.Tag.render`<div style="display: none;"></div>`);
	...
	class ConfirmedEmailDialog extends main_core_events.EventEmitter {
		...
		show() {
			this.DOM.content = main_core.Tag.render`<div>
			<div class="calendar-confirm-email-text">${main_core.Loc.getMessage('EC_CONFIRMED_EMAIL_TEXT_1')}</div>
			<div class="calendar-confirm-email-text"><a class="calendar-confirm-email-help-link" href="javascript:void(0);">${main_core.Loc.getMessage('EC_CONFIRMED_EMAIL_HELP_LINK')}</a></div>
	...
	class EmailLimitationDialog extends main_core_events.EventEmitter {
		...
		show() {
			this.DOM.content = main_core.Tag.render`
			<div>
				<div class="calendar-email-limit-text">${main_core.Loc.getMessage('EC_EMAIL_LIMIT_DENY')}</div>
				<div class="calendar-email-limit-subtext">${main_core.Loc.getMessage('EC_EMAIL_LIMIT_SUBTEXT')}</div>
			</div>
		`;
	...
	class AttendeesList {
		...
		getPopup(menuItems) {
			return main_popup.MenuManager.create(`compact-event-form-attendees${Math.round(Math.random() * 100_000)}`, this.node, menuItems, {
				closeByEsc: true,
				autoHide: true,
				zIndex: this.zIndex,
				offsetTop: 0,
	...
	class IntranetButton {
		...
		isProjectCalendarSlider(slider) {
			if (main_core.Type.isNull(slider)) {
				return false;
			}
		...
		openChatWithConfirm() {
			...
			calendar_util.Util.showConfirmPopup(openChatHandler, main_core.Loc.getMessage('EC_CREATE_CHAT_CONFIRM_QUESTION'), {
		...
		getSlidersToCloseForChat() {
			const openSliders = [...main_sidepanel.SidePanel.Instance.getOpenSliders()];
	...
	class AccessibilitySelector {
		...
		constructor(params) {
			...
			this.input = params.input;
			if (!this.readonly) {
				main_core.Event.bind(this.input, 'click', this.#showMenu);
			}
		}
		#showMenu = () => {
			this.#accessibilityMenu ??= new main_popup.Menu({
...
})(this.BX.Calendar.Controls = this.BX.Calendar.Controls || {}, BX.Calendar, BX, BX.Calendar.Controls, BX.Event, BX.Main, BX.Calendar, BX.Calendar, BX.UI, BX, BX.UI.DatePicker, BX, BX.UI, BX.Calendar, BX.UI, BX.UI.Dialogs, BX.Calendar, BX.UI.EntitySelector, BX.UI, BX.Intranet, BX.SidePanel);
