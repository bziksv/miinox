...
(function (exports, main_core, calendar_controls, calendar_sectioninterface, calendar_util, main_core_events, ui_entitySelector, ui_dialogs_messagebox) {
	'use strict';
	...
	class EditForm extends main_core_events.EventEmitter {
		...
		showSimplePicker(value) {
			...
			moreLinkWrap.appendChild(main_core.Dom.create('SPAN', {
				props: {
	...
	class EditFormRoom extends EditForm {
		...
		getSliderContentWrap() {
			...
			if (sliderContentWrap) {
				...
				sliderContentWrap = this.DOM.outerWrap.appendChild(main_core.Tag.render`
		...
		getFormFieldsWrap(wrap) {
			return wrap.appendChild(main_core.Tag.render`
			...
			return wrap.appendChild(main_core.Tag.render`
				<div class="calendar-field-container calendar-field-container-string">
					<div class="calendar-field-block">
						<input type="text" placeholder="${main_core.Loc.getMessage('EC_SEC_SLIDER_SECTION_TITLE')}" 
					...
					<div class="calendar-field-block">
						<div class ="calendar-list-slider-card-widget-title" style="margin-bottom: 0">
							<span class="calendar-list-slider-card-widget-title-text">
								${main_core.Loc.getMessage('EC_SEC_SLIDER_SECTION_CAPACITY')}
							</span>
		...
		createBottomOptions(wrap) {
			this.DOM.optionsWrap = wrap.appendChild(main_core.Tag.render`
			<div class="calendar-list-slider-new-calendar-options-container"></div>`);
			this.initSectionColorSelector();
			this.initAccessController();
		}
		createButtons(wrap) {
			this.buttonsWrap = wrap.appendChild(main_core.Tag.render`
	...
	class RoomsInterface extends calendar_sectioninterface.SectionInterface {
		...
		renderTitleWrap() {
			...
			<div class="calendar-list-slider-title-container">
				<div class="calendar-list-slider-title">
					...
					<span onclick="${this.openHelpDesk.bind(this)}" class="ui-hint" title="${main_core.Loc.getMessage('EC_CALENDAR_HOW_DOES_IT_WORK')}">
		...
		renderRoomFormWrap() {
			return main_core.Tag.render`
				<div class="calendar-list-slider-card-widget calendar-list-slider-form-wrap">
					<div class="calendar-list-slider-card-widget-title">
						<span class="calendar-list-slider-card-widget-title-text">${main_core.Loc.getMessage('EC_SEC_SLIDER_NEW_ROOM')}</span>
					</div>
				</div>
		`;
		}
		...
		setBlocksWrap() {
			if (this.DOM.blocksWrap) {
				...
				this.DOM.blocksWrap = this.DOM.outerWrap.appendChild(main_core.Tag.render`
					<div></div>
				`);
			}
		}
		...
		createBlockWrap(wrap) {
			const listWrap = wrap.appendChild(main_core.Tag.render`
		...
		renderCategoryBlockTitle(category) {
			return main_core.Tag.render`
				<div class="calendar-list-slider-card-widget-title-text calendar-list-slider-item-category-text" 
					title="${main_core.Text.encode(category.name)}"
				>
					${main_core.Text.encode(category.name)}
				</div>
		`;
		}
		createCategoryBlockContent(category, wrap) {
			if (category.rooms.length) {
				...
				wrap.appendChild(main_core.Tag.render`
					<li class="calendar-list-slider-card-widget-title-text">${main_core.Loc.getMessage('EC_CATEGORY_EMPTY')}</li>
				`);
			}
		}
		...
		renderRoomBlockTitle(room) {
			return main_core.Tag.render`
				<div class="calendar-list-slider-item-name" title="${main_core.Text.encode(room.name)}">
					${main_core.Text.encode(room.name)}
				</div>
		`;
		}
		...
		showRoomDeleteConfirm(room) {
			...
			this.DOM.confirmRoomPopup = new ui_dialogs_messagebox.MessageBox({
				useAirDesign: true,
				message: main_core.Loc.getMessage('EC_ROOM_DELETE_CONFIRM'),
				minHeight: 120,
				minWidth: 280,
				maxWidth: 300,
		...
		showCategoryDeleteConfirm(category) {
			...
			this.DOM.confirmCategoryPopup = new ui_dialogs_messagebox.MessageBox({
				useAirDesign: true,
				message: main_core.Loc.getMessage('EC_CATEGORY_DELETE_CONFIRM'),
				minHeight: 120,
				minWidth: 280,
				maxWidth: 300,
				buttons: BX.UI.Dialogs.MessageBoxButtons.OK_CANCEL,
...
})(this.BX.Calendar.Rooms = this.BX.Calendar.Rooms || {}, BX, BX.Calendar.Controls, BX.Calendar, BX.Calendar, BX.Event, BX.UI.EntitySelector, BX.UI.Dialogs);
