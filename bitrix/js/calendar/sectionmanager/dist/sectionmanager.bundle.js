/* eslint-disable */
this.BX = this.BX || {};
(function (exports, calendar_util, main_core, calendar_sectionmanager, main_core_events) {
	'use strict';
	...
	class CalendarTaskSection extends CalendarSection {
		constructor(data = {}, {
			...
			if (type === 'user' && userId === ownerId) {
				...
				defaultName = isNewProjectsOn ? main_core.Loc.getMessage('EC_SEC_PROJECT_TASK_DEFAULT') : main_core.Loc.getMessage('EC_SEC_GROUP_TASK_DEFAULT');
			}
			super({
				ID: 'tasks',
	...
	class SectionManager {
		...
		static getSectionGroupList(options = {}) {
			...
			if (type === 'user') {
				...
				const collabKey = options.isNewProjectsOn ? 'EC_SEC_SLIDER_PROJECT_CALENDARS_LIST' : 'EC_SEC_SLIDER_COLLAB_CALENDARS_LIST';
				const groupTitleMessage = isCollabUser || isCollabContext ? collabKey : 'EC_SEC_SLIDER_GROUP_CALENDARS_LIST';
			...
			const collabTitle = options.isNewProjectsOn ? BX.message('EC_SEC_SLIDER_POPUP_MENU_ADD_PROJECT') : BX.message('EC_SEC_SLIDER_POPUP_MENU_ADD_COLLAB');
			sectionGroups.push({
				title: collabTitle,
				type: 'collab'
			});
...
})(this.BX.Calendar = this.BX.Calendar || {}, BX.Calendar, BX, BX.Calendar, BX.Event);
