...
(function (exports, ui_designTokens, main_core, main_popup, ui_buttons, main_core_events, ui_tour, calendar_util) {
	'use strict';
	...
	class ConnectionProvider extends main_core_events.EventEmitter {
		...
		ERROR_CODE = 'error';
		STATUS_LIST = [this.STATUS_SYNCHRONIZING, this.STATUS_SUCCESS, this.STATUS_FAILED, this.STATUS_REFUSED, this.STATUS_PENDING, this.STATUS_NOT_CONNECTED];
	...
	class SyncStatusPopupV2 extends main_core_events.EventEmitter {
		...
		static getFailedConnectionsFromProvider(provider) {
			return provider.getConnections()?.filter(connection => {
			...
			const popup = new main_popup.Popup({
				...
				<div class="calendar-sync-popup-status-refused">
					<div class="calendar-sync-popup-status-refused-title">
						${main_core.Loc.getMessage('CAL_SYNC_INFO_STATUS_REFUSED_POPUP_TITLE')}
					</div>
					<div class="calendar-sync-popup-status-refused-text">
						${main_core.Loc.getMessage('CAL_SYNC_INFO_STATUS_REFUSED_POPUP_TEXT')}
					</div>
				</div>
			`,
				width: 400,
				...
				case 'failed':
					{
						return {
							...
							collapsedIcon: ui_buttons.ButtonIcon.EXCLAMATION,
				...
				case 'synchronizing':
					{
						return {
							...
							color: ui_buttons.ButtonColor.LIGHT_BORDER,
							style: ui_buttons.AirButtonStyle.OUTLINE,
							collapsedIcon: ui_buttons.ButtonIcon.CALENDAR_WITH_CHECKS,
	...
	class SyncStatusPopup extends main_core_events.EventEmitter {
		...
		getContentRefreshBlock() {
			this.footerWrapper = main_core.Tag.render`
		...
		getContentRefreshButton() {
			this.refreshButton = main_core.Tag.render`
			<button class="calendar-sync-popup-footer-btn">${main_core.Loc.getMessage('CAL_REFRESH')}</button>
		`;
		...
		getRefreshStatus() {
			this.refreshStatusBlock = main_core.Tag.render`
			<span class="calendar-sync-popup-footer-status">${main_core.Loc.getMessage('CAL_REFRESH_JUST')}</span>
		`;
	...
	class Manager extends main_core_events.EventEmitter {
		...
		processSyncConnection(params) {
			for (let providerName in this.connectionsProviders) {
				if (this.connectionsProviders.hasOwnProperty(providerName) && this.connectionsProviders[providerName].getWizardSyncMode() && providerName === params?.vendorName) {
...
})(this.BX.Calendar.Sync.Manager = this.BX.Calendar.Sync.Manager || {}, BX, BX, BX.Main, BX.UI, BX.Event, BX.UI.Tour, BX.Calendar);
