...
(function (exports, main_core, main_core_events, main_popup, ui_userfield) {
	'use strict';
	 ...
	 */
	class FieldTypes {
		static isRichTextEnabled() {
			return main_core.Extension.getSettings('ui.userfieldfactory').get('richTextEnabled', false) === true;
		}
		static getTypes() {
			const types = {
				string: 'string',
		...
		static getDescriptions() {
			...
			if (FieldTypes.isRichTextEnabled()) {
				descriptions[FieldTypes.getTypes().richText] = {
					title: main_core.Loc.getMessage("UI_USERFIELD_FACTORY_UF_RICH_TEXT_TITLE"),
					description: main_core.Loc.getMessage("UI_USERFIELD_FACTORY_UF_RICH_TEXT_LEGEND"),
					defaultTitle: main_core.Loc.getMessage('UI_USERFIELD_FACTORY_UF_RICH_TEXT_LABEL')
				};
			}
	...
	class CreationMenu {
		...
		render(onItemClick) {
			if (!this.container) {
				this.container = main_core.Tag.render`<div class="ui-userfieldfactory-creation-menu-container"></div>`;
				...
				this.topScrollButton = main_core.Tag.render`<div class="ui-userfieldfactory-creation-menu-scroll-top">${scrollIcon}</div>`;
				this.bottomScrollButton = main_core.Tag.render`<div class="ui-userfieldfactory-creation-menu-scroll-bottom">${scrollIcon}</div>`;
		...
		renderList(onItemClick) {
			if (!this.containerList) {
				this.containerList = main_core.Tag.render`<div class="ui-userfieldfactory-creation-menu-list"></div>`;
		...
		renderItem(item, onClick) {
			return main_core.Tag.render`<div class="ui-userfieldfactory-creation-menu-item" onclick="${() => {
	...
	class Configurator {
		...
		render() {
			this.node = main_core.Tag.render`<div class="ui-userfieldfactory-configurator"></div>`;
			this.labelInput = main_core.Tag.render`<input class="ui-ctl-element" type="text" placeholder="${main_core.Text.encode(this.userField.getTitle())}" />`;
			this.node.appendChild(main_core.Tag.render`<div class="ui-userfieldfactory-configurator-block">
			<div class="ui-userfieldfactory-configurator-title">
				<span class="ui-userfieldfactory-configurator-title-text">${main_core.Loc.getMessage('UI_USERFIELD_FACTORY_CONFIGURATOR_FIELD_TITLE')}</span>
			</div>
			...
			this.saveButton = main_core.Tag.render`<span class="ui-btn ui-btn-primary" onclick="${save.bind(this)}">${main_core.Loc.getMessage('UI_USERFIELD_SAVE')}</span>`;
			this.cancelButton = main_core.Tag.render`<span class="ui-btn ui-btn-light-border" onclick="${cancel.bind(this)}">${main_core.Loc.getMessage('UI_USERFIELD_CANCEL')}</span>`;
			this.node.appendChild(main_core.Tag.render`<div class="ui-userfieldfactory-configurator-block">
		...
		renderEnumeration() {
			this.enumItemsContainer = main_core.Tag.render`<div class="ui-userfieldfactory-configurator-block"></div>`;
			this.enumAddItemContainer = main_core.Tag.render`<div class="ui-userfieldfactory-configurator-block-add-field">
			<span class="ui-userfieldfactory-configurator-add-button" onclick="${() => {
			this.addEnumInput().focus();
		}}">${main_core.Loc.getMessage('UI_USERFIELD_ADD')}</span>
		</div>`;
			this.enumContainer = main_core.Tag.render`<div class="ui-userfieldfactory-configurator-block">
			<div class="ui-userfieldfactory-configurator-title">
				<span class="ui-userfieldfactory-configurator-title-text">${main_core.Loc.getMessage('UI_USERFIELD_FACTORY_UF_ENUM_ITEMS')}</span>
			</div>
		...
		addEnumInput(item) {
			...
			const node = main_core.Tag.render`<div style="margin-bottom: 10px;" class="ui-ctl ui-ctl-textbox ui-ctl-w100 ui-ctl-row">
			<input class="ui-ctl-element" type="text" value="${main_core.Text.encode(enumItem.getValue())}">
		...
		renderOptions() {
			this.optionsContainer = main_core.Tag.render`<div class="ui-userfieldfactory-configurator-block"></div>`;
			if (this.canRequiredFields) {
				this.mandatoryCheckbox = main_core.Tag.render`<input class="ui-ctl-element" type="checkbox">`;
				this.mandatoryCheckbox.checked = this.userField.isMandatory();
				this.optionsContainer.appendChild(main_core.Tag.render`<div>
				<label class="ui-ctl ui-ctl-checkbox ui-ctl-xs">
					${this.mandatoryCheckbox}
					<div class="ui-ctl-label-text">${main_core.Loc.getMessage('UI_USERFIELD_FACTORY_FIELD_REQUIRED')}</div>
				</label>
			</div>`);
			}
			if (!this.userField.isSaved() && (this.userField.getUserTypeId() === FieldTypes.getTypes().date || this.userField.getUserTypeId() === FieldTypes.getTypes().datetime)) {
				this.timeCheckbox = main_core.Tag.render`<input class="ui-ctl-element" type="checkbox">`;
			...
			`;
				if (this.userField.getUserTypeId() === FieldTypes.getTypes().datetime) {
					main_core.Dom.append(main_core.Tag.render`<div className="ui-ctl-label-text">${main_core.Loc.getMessage('UI_USERFIELD_FACTORY_UF_ENABLE_TIME')}</div>`, label);
				}
				this.optionsContainer.appendChild(main_core.Tag.render`<div>
			...
			if (!this.userField.isSaved() && this.userField.getUserTypeId() !== FieldTypes.getTypes().boolean && this.canMultipleFields) {
				this.multipleCheckbox = main_core.Tag.render`<input class="ui-ctl-element" type="checkbox">`;
				this.multipleCheckbox.checked = this.userField.isMultiple();
				this.optionsContainer.appendChild(main_core.Tag.render`<div>
				<label class="ui-ctl ui-ctl-checkbox ui-ctl-xs">
					${this.multipleCheckbox}
					<div class="ui-ctl-label-text">${main_core.Loc.getMessage('UI_USERFIELD_FACTORY_FIELD_MULTIPLE')}</div>
				</label>
			</div>`);
			}
...
})(this.BX.UI.UserFieldFactory = this.BX.UI.UserFieldFactory || {}, BX, BX.Event, BX.Main, BX.UI.UserField);
