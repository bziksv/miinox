/* eslint-disable */
this.BX = this.BX || {};
(function (exports, main_core, calendar_util, main_core_events, ui_infoHelper, calendar_ui_tools_draganddrop, main_popup, main_date, ui_avatar) {
	'use strict';
	...
	class Selector extends main_core_events.EventEmitter {
		...
		render() {
			this.DOM.timeNodes = {};
			this.DOM.timeWrap = main_core.Tag.render`
			<div class="calendar-planner-selector-notices-container"></div>
		`;
			this.DOM.wrap = main_core.Tag.render`
		...
		`;
			...
			this.DOM.titleNode = main_core.Tag.render`<div class="calendar-planner-selector-notice" style="display: none"></div>`;
		}
		renderMoreButton() {
			this.DOM.moreButton = main_core.Tag.render`
		...
		showTimeNode(offsetTop, time, timezone, entryId, isWarning = false) {
			...
			this.DOM.timeNodes[offsetTop] = main_core.Tag.render`
			<div 
		...
		transit(params = {}) {
			...
			let fromX = params.fromX ?? parseInt(this.DOM.wrap.style.left),
	...
	class Planner extends main_core_events.EventEmitter {
		...
		setConfig(params) {
			...
			const warningTimeFrom = calendar_util.Util.config?.work_time_start ?? 9;
			const warningTimeTo = calendar_util.Util.config?.work_time_end ?? 18;
		...
		render() {
			...
			this.DOM.mainWrap = main_core.Tag.render`
			<div
		...
		renderEntriesOuterWrap() {
			this.DOM.entriesOuterWrap = main_core.Tag.render`
			<div
				...
				<div class="calendar-planner-general-info">
					<div class="calendar-planner-users-header">
						<span class="calendar-planner-users-item">
							${main_core.Loc.getMessage('EC_PL_ATTENDEES_TITLE') + ' '}
		...
		renderEntriesListTitleCounter() {
			this.entriesListTitleCounter = main_core.Tag.render`
			<span></span>
		`;
			return this.entriesListTitleCounter;
		}
		renderEntriesListWrap() {
			this.DOM.entrieListWrap = main_core.Tag.render`
			<div
		...
		renderTimelineFixedWrap() {
			this.DOM.timelineFixedWrap = main_core.Tag.render`
		...
		renderTimelineVerticalConstraint() {
			this.DOM.timelineVerticalConstraint = main_core.Tag.render`
		...
		renderTimelineInnerWrap() {
			this.DOM.timelineInnerWrap = main_core.Tag.render`
		...
		renderTimelineScaleWrap() {
			this.DOM.timelineScaleWrap = main_core.Tag.render`
		...
		renderTimelineDataWrap() {
			this.DOM.timelineDataWrap = main_core.Tag.render`
		...
		renderTimelineAccessibilityWrap() {
			this.DOM.accessibilityWrap = main_core.Tag.render`
		...
		renderSettingsButton() {
			...
			this.DOM.settingsButton = main_core.Tag.render`
			<div class="calendar-planner-settings-icon-container" title="${main_core.Loc.getMessage('EC_PL_SETTINGS_SCALE')}">
		...
		renderSelectorPopup() {
			this.DOM.selectorPopup = main_core.Tag.render`
		...
		renderTimezoneNoticeCount() {
			this.DOM.timezoneNoticeCount = main_core.Tag.render`
		...
		renderVacationNode(entryId) {
			const vacationNode = main_core.Tag.render`
			<div 
			class="calendar-planner-timeline-side-notice --vacation"
			id="timeline-side-notice-${entryId}"
			style="display: none;"
			>${main_core.Loc.getMessage('EC_PLANNER_IN_VACATION')}</div>
		`;
		...
		showHintPopup(node) {
			node.hintPopup = main_core.Tag.render`
				...
				for (let i = 0; i < this.scaleData.length; i++) {
					if (this.showTimelineDayTitle && !this.isOneDayScale()) {
						if (this.scaleDayTitles[this.scaleData[i].daystamp]) {
							...
							outerDayCont = this.DOM.timelineScaleWrap.appendChild(main_core.Tag.render`
						...
						`);
							...
							//F d, l
							dayTitle = outerDayCont.appendChild(main_core.Tag.render`
							<div class="${dayTitleClass}">
								<span>
									${main_date.DateTimeFormat.format(main_date.DateTimeFormat.getFormat('DAY_OF_WEEK_MONTH_FORMAT'), this.scaleData[i].timestamp / 1000)}
								</span>
						...
						`);
							...
							if (date.getTime() === today.getTime() && this.isTodayButtonEnabled()) {
								this.todayTitleButton = dayTitle.firstElementChild.appendChild(main_core.Tag.render`
							...
							cont = outerDayCont.appendChild(main_core.Tag.render`
					...
					if (!this.isOneDayScale() && this.scaleData[i + 1] && this.scaleData[i + 1].dayStart) {
						cont.appendChild(main_core.Tag.render`
		...
		buildTodayButtonWrap() {
			...
			const todayButton = this.DOM.entriesOuterWrap.appendChild(main_core.Tag.render`
		...
		`);
			...
			this.DOM.todayButtonContainer = this.DOM.timelineScaleWrap.appendChild(main_core.Tag.render`
			<div class="calendar-planner-today-button-container" style="width: ${width}px; left: ${left}px; top: ${top}px;"></div>
		`);
			this.todayButton = this.DOM.todayButtonContainer.appendChild(main_core.Tag.render`
			<div class="calendar-planner-today-button" style="width: ${this.todayButtonWidth}px; direction: rtl;">${this.todayLocMessage}</div>
		`);
			this.todayRightButton = this.DOM.timelineVerticalConstraint.appendChild(main_core.Tag.render`
		...
		displayEntryRow(entry, accessibility = []) {
			...
			if (entry.type === 'moreLink') {
				rowWrap = this.DOM.entrieListWrap.appendChild(main_core.Tag.render`
			...
			`);
				...
				if (this.showEntryName) {
					this.DOM.showMoreUsersLink = rowWrap.appendChild(main_core.Tag.render`
			...
			`);
				...
				if (this.showEntryName) {
					rowWrap.append(main_core.Tag.render`
			...
			`);
				if (this.showEntryName) {
					rowWrap.appendChild(main_core.Tag.render`
					<span class="calendar-planner-user-name"></span>
				`).appendChild(main_core.Tag.render`
				...
				`);
					...
					rowWrap.appendChild(main_core.Tag.render`
					<div class="calendar-planner-location-image-icon" title="${main_core.Text.encode(entry.name)}"></div>
				`);
				}
			} else if (entry.type === 'resource') {
				if (!this.entriesResourceListWrap || !BX.isNodeInDom(this.entriesResourceListWrap)) {
					this.entriesResourceListWrap = this.DOM.entrieListWrap.appendChild(main_core.Tag.render`
					<div class="calendar-planner-container-resource">
						<div class="calendar-planner-resource-header">
							<span class="calendar-planner-users-item">${main_core.Loc.getMessage('EC_PL_RESOURCE_TITLE')}</span>
						</div>
					</div>
				`);
				}
				rowWrap = this.entriesResourceListWrap.appendChild(main_core.Tag.render`
			...
			`);
				rowWrap.appendChild(main_core.Tag.render`
				<div class="calendar-planner-all-users">${main_core.Text.encode(entry.name)}</div>
			`);
			}
			let top = rowWrap.offsetTop + 13;
			let dataRowWrap = this.DOM.accessibilityWrap.appendChild(main_core.Tag.render`
		...
		static getEntryAvatarNode(entry) {
			...
			if (entry.isCollabUser) {
				...
				<div 
					...
					bx-tooltip-context="b24"
					bx-tooltip-classname="calendar-planner-user-tooltip" 
					title="${main_core.Text.encode(entry.name)}" class="ui-icon calendar-planner-user-image-icon ${defaultAvatarClass}"
				>
					<i></i>
				</div>
			`;
				...
				<div 
					...
					bx-tooltip-context="b24"
					bx-tooltip-classname="calendar-planner-user-tooltip" 
					title="${main_core.Text.encode(entry.name)}" 
		...
		selectEntryRow(entry) {
			if (BX.type.isPlainObject(entry)) {
				...
				if (!entry.selectWrap || !BX.isParentForNode(this.selectedEntriesWrap, entry.selectWrap)) {
					entry.selectWrap = this.selectedEntriesWrap.appendChild(main_core.Tag.render`
		...
		showSettingsPopup() {
			let settingsDialogCont = main_core.Tag.render`<div class="calendar-planner-settings-popup"></div>`;
			let scaleRow = settingsDialogCont.appendChild(main_core.Tag.render`
			<div class="calendar-planner-settings-row">
				<i>${main_core.Loc.getMessage('EC_PL_SETTINGS_SCALE')}:</i>
			</div>
		`);
			let scaleWrap = scaleRow.appendChild(main_core.Tag.render`
		...
		`);
			...
			this.scaleTypes.forEach(scale => {
				scaleWrap.appendChild(main_core.Tag.render`<span class="calendar-planner-option-tab ${scale === this.scaleType ? ' calendar-planner-option-tab-active' : ''}" data-bx-planner-scale="${scale}">${main_core.Loc.getMessage('EC_PL_SETTINGS_SCALE_' + scale.toUpperCase())}</span>`);
			});
		...
		showProposeControl() {
			if (!this.DOM.proposeTimeButton) {
				this.DOM.proposeTimeButton = this.DOM.mainWrap.appendChild(main_core.Tag.render`
		...
		lock() {
			if (!this.DOM.lockScreen) {
				this.DOM.lockScreen = main_core.Tag.render`
				<div class="calendar-planner-timeline-locker">
					<div class="calendar-planner-timeline-locker-container">
						<div class="calendar-planner-timeline-locker-top">
							...
							<div class="calendar-planner-timeline-text">${main_core.Loc.getMessage('EC_PL_LOCKED_TITLE')}</div>
						</div>
		...
		getLockerButton() {
			if (!this.DOM.lockButton) {
				this.DOM.lockButton = main_core.Tag.render`
				<div class="calendar-planner-timeline-locker-button">
					<div class="ui-btn ui-btn-sm ui-btn-light-border ui-btn-round">${main_core.Loc.getMessage('EC_PL_UNLOCK_FEATURE')}</div>
				</div>
			`;
...
})(this.BX.Calendar = this.BX.Calendar || {}, BX, BX.Calendar, BX.Event, BX.UI, BX.Calendar.Ui.Tools, BX.Main, BX.Main, BX.UI);
