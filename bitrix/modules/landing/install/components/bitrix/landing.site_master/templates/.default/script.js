...
(function (exports, main_core) {
	...
	class SiteMasterA11y {
		...
		announce(message, politeness = 'polite') {
			if (!main_core.Type.isStringFilled(message) || this.isDuplicate(message)) {
				return;
			}
		...
		rememberTrigger(node) {
			this.trigger = main_core.Type.isDomNode(node) ? node : null;
		}
		restoreTrigger() {
		 ...
		 */
		bindSliderTriggers() {
			document.querySelectorAll(sliderTriggerSelector).forEach(trigger => {
				main_core.Event.bind(trigger, 'click', event => {
	...
	const siteMasterA11y = new SiteMasterA11y();

	exports.SiteMasterA11y = SiteMasterA11y;
	exports.siteMasterA11y = siteMasterA11y;
