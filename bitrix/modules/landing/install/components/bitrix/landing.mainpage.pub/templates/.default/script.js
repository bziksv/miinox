...
(function (exports, main_core, landing_metrika, ui_lottie) {
	...
	class Analytics {
		...
		initEventListeners() {
			...
			for (const block of blocks) {
				...
				elements.forEach(element => {
					main_core.Event.bind(element, 'click', event => this.onClick(event, code));
				});
			}
		}
		 ...
		 */
		findClickableElements(block) {
			const elements = [...block.querySelectorAll(`${TAG_A}, ${TAG_BUTTON}, [${DATA_PSEUDO_URL}], #${FEEDBACK_BUTTON_ID}`)];
			return elements.filter(el => this.isClickableElement(el));
		}
		 ...
		 */
		getBlockCode(block) {
			const className = [...block.classList].find(name => name.startsWith(BLOCK_PREFIX) && name !== BLOCK_WRAPPER_CLASS);
			if (!className) {
				return '';
			}
		 ...
		 */
		extractHrefFromPseudoUrl(target) {
	...
	const renderLottieAnimation = target => {
		if (!target) {
			console.error('Target container is not specified for Lottie animation.');
			return;
		}
		const animation = ui_lottie.Lottie.loadAnimation({
			container: target,
			renderer: 'svg',
			loop: false,
			autoplay: true,
