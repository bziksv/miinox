...
(function (exports, main_core, landing_loc, landing_main, landing_metrika, landing_ui_panel_stylepanel, landing_ui_field_textfield, landing_imageuploader, landing_ui_button_basebutton, landing_ui_button_aiimagebutton, landing_env) {
	'use strict';

	class Image extends landing_ui_field_textfield.TextField {
		copilotBindElement = null;
		aiButton = null;
		copilotContext = null;
		...
		constructor(data) {
			...
			this.isAiImageAvailable = main_core.Type.isBoolean(data.isAiImageAvailable) ? data.isAiImageAvailable : false;
			this.isAiImageActive = main_core.Type.isBoolean(data.isAiImageActive) ? data.isAiImageActive : false;
			this.aiUnactiveInfoCode = main_core.Type.isString(data.aiUnactiveInfoCode) ? data.aiUnactiveInfoCode : null;
			...
			main_core.Dom.attr(this.layout, "data-testid", "landing-field-image");
			...
			this.linkInput.onInputHandler = main_core.Runtime.debounce(this.onLinkInput.bind(this), 777);
			...
			main_core.Event.bind(this.dropzone, "keydown", this.onDropzoneKeydown.bind(this));
			...
			this.hiddenImage = main_core.Dom.create("img", {
				props: {
					className: "landing-ui-field-image-hidden"
				}
			});
			if (main_core.Type.isPlainObject(this.content) && "src" in this.content) {
			...
			this.onEditorClose = main_core.Runtime.debounce(this.onEditorClose, 1000, this);
			this.right = Image.createRightLayout();

			// ai images
			if (this.isAiImageAvailable && (this.type === "background" || this.type === "image")) {
				...
				this.stylePanel = landing_ui_panel_stylepanel.StylePanel.getInstance().layout;
				this.stylePanelContent = landing_ui_panel_stylepanel.StylePanel.getInstance().content;
				this.copilotContext.Runtime.loadExtension('ai.copilot').then(({
					Copilot,
					CopilotEvents
				}) => {
					...
					main_core.Event.bind(this.stylePanelContent, 'scroll', this.onScrollContentPanel.bind(this));
					main_core.Event.bind(this.stylePanel, 'click', this.onClickStylePanel.bind(this));
					main_core.Event.EventEmitter.subscribe('BX.Landing.UI.Panel.ContentEdit:onClick', this.onClickContentPanel.bind(this));
					main_core.Event.EventEmitter.subscribe('BX.Landing.UI.Panel.BasePanel:onHide', this.closeCopilot.bind(this));
					main_core.Event.EventEmitter.subscribe('BX.Landing.UI.Panel.BasePanel:onClick', this.onClickContentPanel.bind(this));
					main_core.Event.EventEmitter.subscribe('BX.Landing.UI.Panel.BasePanel:onScroll', this.onScrollContentPanel.bind(this));
			...
			if (this.content.type === "icon") {
				...
				this.showPreview();
				this.altField.layout.hidden = true;
				main_core.Dom.addClass(this.layout, 'landing-ui-field-image-icon');
			}
			this.makeAsLinkWrapper = main_core.Dom.create("div", {
				props: {
			...
			this.uploader = new landing_imageuploader.ImageUploader({
		...
		static createLinkInput() {
			...
			main_core.Dom.attr(field.layout, "data-testid", "landing-field-image-link");
			main_core.Dom.attr(field.input, "data-testid", "landing-field-image-link-input");
			return field;
		}
		...
		static createDropzone(id) {
			return main_core.Dom.create("label", {
				...
				attrs: {
					...
					"role": "button",
					"tabindex": "0",
					"data-testid": "landing-field-image-dropzone"
				}
			});
		}
		...
		static createClearButton() {
			return new landing_ui_button_basebutton.BaseButton("clear", {
				...
				attrs: {
					"aria-label": landing_loc.Loc.getMessage("LANDING_FIELD_IMAGE_CLEAR"),
					"data-testid": "landing-field-image-clear-btn"
				}
			});
		}
		...
		static createImagePreview() {
			return main_core.Dom.create("div", {
				...
				attrs: {
					"data-testid": "landing-field-image-preview"
				}
			});
		}
		...
		static createAltField() {
			...
			main_core.Dom.attr(field.layout, "data-testid", "landing-field-image-alt");
			// the editable element is a contenteditable div, not an <input>
			main_core.Dom.attr(field.input, "data-testid", "landing-field-image-alt-input");
			return field;
		}
		 ...
		 * @return {BaseButton}
		 */
		static createAiButton(compactMode = false) {
			return new landing_ui_button_aiimagebutton.AiImageButton("ai", {
				...
				attrs: {
					"data-testid": "landing-field-image-ai-btn"
				}
			});
		}
		...
		static createUploadButton() {
			return new landing_ui_button_basebutton.BaseButton("upload", {
				...
				attrs: {
					"data-testid": "landing-field-image-upload-btn"
				}
			});
		}
		...
		static createEditButton() {
			var field = new landing_ui_button_basebutton.BaseButton("edit", {
				...
				className: "landing-ui-field-image-action-button",
				attrs: {
					"data-testid": "landing-field-image-edit-btn"
				}
			});
			return field;
		}
		...
		onFileChange(file) {
			this.showLoader();
			this.upload(file).then(this.applyUploadResult.bind(this)).then(this.hideLoader.bind(this)).catch(function (err) {
				console.error(err);
				this.hideLoader();
			}.bind(this));
		}
		...
		imageCopilotImageResultHandler(e) {
			...
			main_core.Dom.style(this.preview, 'background-image', `url("${this.imageCopilotUrl}")`);
		...
		imageCopilotSaveImageHandler() {
			...
			const name = url.searchParams.get('hashId') ?? main_core.Text.getRandom(8);
			BX.Landing.Utils.urlToBlob(url.href).then(blob => {
				const ext = blob.type === 'image/png' ? '.png' : '.jpg';
				blob.lastModifiedDate = new Date();
				blob.name = `${name}.${ext}`;
				return blob;
			}).then(this.upload.bind(this)).then(this.applyUploadResult.bind(this)).then(this.hideLoader.bind(this)).then(() => {
		...
		imageCopilotCancelImageHandler() {
			if (this.copilotBindElement === this.dropzone) {
				...
				main_core.Dom.removeClass(this.preview, '--shown');
				main_core.Dom.style(this.preview, 'background-image', `url("${this.input.innerText.trim()}")`);
			}
		}
		closeCopilot() {
			...
			if (!this.uploadMenu) {
				this.uploadMenu = BX.Main.MenuManager.create({
					...
					items: [{
						...
						attrs: {
							"data-testid": "landing-field-image-menu-unsplash"
						}
					}, ...(landing_env.Env.getInstance().getOptions()['google_images_available'] ? [{
						text: landing_loc.Loc.getMessage("LANDING_IMAGE_UPLOAD_MENU_GOOGLE"),
						onclick: this.onGoogleShow.bind(this),
						attrs: {
							"data-testid": "landing-field-image-menu-google"
						}
					}] : []),
					// {
		...
		onUnsplashShow() {
			...
			BX.Landing.UI.Panel.Image.getInstance().show("unsplash", this.dimensions, this.loader, this.uploadParams).then(this.upload.bind(this)).then(this.applyUploadResult.bind(this)).then(this.hideLoader.bind(this)).catch(function (err) {
				console.error(err);
				this.hideLoader();
			}.bind(this));
		}
		onGoogleShow() {
			this.uploadMenu.close();
			BX.Landing.UI.Panel.Image.getInstance().show("google", this.dimensions, this.loader, this.uploadParams).then(this.upload.bind(this)).then(this.applyUploadResult.bind(this)).then(this.hideLoader.bind(this)).catch(function (err) {
		...
		onLinkInput(value) {
			const tmpImage = main_core.Dom.create("img");
			tmpImage.src = value;
			tmpImage.onload = () => {
				this.showPreview();
		...
		upload(file, additionalParams) {
			...
			this.showLoader();
			const isPng = main_core.Type.isStringFilled(file.type) && file.type.includes('png');
			const isSvg = main_core.Type.isStringFilled(file.type) && file.type.includes('svg');
...
})(this.BX.Landing.UI.Field = this.BX.Landing.UI.Field || {}, BX, BX.Landing, BX.Landing, BX.Landing, BX.Landing.UI.Panel, BX.Landing.UI.Field, BX.Landing, BX.Landing.UI.Button, BX.Landing.UI.Button, BX.Landing);
