...
(function (exports, main_core, landing_loc, landing_env, landing_main, landing_ui_a11y, landing_ui_form_baseform, landing_ui_form_menuitemform, ui_draganddrop_draggable) {
  'use strict';
   ...
   */
  var MenuForm = /*#__PURE__*/function (_landing_ui_form_base) {
    function MenuForm() {
      var _this;
      ...
      main_core.Event.bind(_this.getBody(), 'keydown', _this.onDragHandleKeyDown);
      _this.onMenuItemRemove = _this.onMenuItemRemove.bind(_this);
      main_core.Dom.append(_this.getAddItemLayout(), _this.layout);
      return _this;
    }
    babelHelpers.inherits(MenuForm, _landing_ui_form_base);
    return babelHelpers.createClass(MenuForm, [{
      key: "addForm",
      ...
      value: function onDragHandleKeyDown(event) {
        ...
        var handle = event.target.closest('.landing-ui-form-header-drag-button');
        if (!handle) {
          return;
        }
        var itemLayout = handle.closest('.landing-ui-form-menuitem');
        var form = itemLayout ? this.forms.getByLayout(itemLayout) : null;
        if (!form) {
          return;
        }
      ...
      key: "moveMenuItem",
      value: function moveMenuItem(form, direction) {
        var elements = this.draggable.getDraggableElements();
        var sourceElement = form.layout;
        var sourceIndex = elements.indexOf(sourceElement);
        if (sourceIndex === -1) {
          return;
        }
        var depth = this.draggable.getElementDepth(sourceElement);
        var sourceBlock = this.getSiblingBlockRange(elements, sourceIndex, depth);
        var targetStart = direction === 'up' ? this.findPreviousSiblingStart(elements, sourceBlock.start, depth) : this.findNextSiblingStart(elements, sourceBlock.end, depth);
        if (targetStart === -1) {
          landing_ui_a11y.A11y.announce(landing_loc.Loc.getMessage(direction === 'up' ? 'LANDING_MENUITEM_MOVE_BLOCKED_TOP' : 'LANDING_MENUITEM_MOVE_BLOCKED_BOTTOM').replace('#TITLE#', function () {
            return form.title;
          }));
          return;
        }
        var sourceElements = elements.slice(sourceBlock.start, sourceBlock.end + 1);
        if (direction === 'up') {
          var anchor = elements[targetStart];
          sourceElements.forEach(function (element) {
            main_core.Dom.insertBefore(element, anchor);
          });
        } else {
          var targetBlock = this.getSiblingBlockRange(elements, targetStart, depth);
          var cursor = elements[targetBlock.end];
          sourceElements.forEach(function (element) {
            main_core.Dom.insertAfter(element, cursor);
            cursor = element;
          });
        }
        this.draggable.invalidateCache();
        var handle = sourceElement.querySelector('.landing-ui-form-header-drag-button');
        if (handle) {
          handle.focus();
        }
        landing_ui_a11y.A11y.announce(landing_loc.Loc.getMessage(direction === 'up' ? 'LANDING_MENUITEM_MOVED_UP' : 'LANDING_MENUITEM_MOVED_DOWN').replace('#TITLE#', function () {
          return form.title;
        }));
      }
    }, {
      key: "getSiblingBlockRange",
      value: function getSiblingBlockRange(elements, index, depth) {
        var end = index;
      ...
      key: "findPreviousSiblingStart",
      value: function findPreviousSiblingStart(elements, blockStart, depth) {
        var i = blockStart - 1;
      ...
      key: "findNextSiblingStart",
      value: function findNextSiblingStart(elements, blockEnd, depth) {
        var next = blockEnd + 1;
      ...
      value: function serialize() {
        ...
        var _getChildren = function getChildren(parent) {
...
})(this.BX.Landing.UI.Form = this.BX.Landing.UI.Form || {}, BX, BX.Landing, BX.Landing, BX.Landing, BX.Landing.UI, BX.Landing.UI.Form, BX.Landing.UI.Form, BX.UI.DragAndDrop);
