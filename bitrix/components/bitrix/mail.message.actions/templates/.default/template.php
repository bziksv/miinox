...
use Bitrix\UI\Toolbar\Facade\Toolbar;use Bitrix\Main\Web\Uri;
...
$createMenu = array(
	'TASKS_TASK' => array(
		...
		'href' => (string)(new Uri(\CComponentEngine::makePathFromTemplate(
				...
				'MAIL_SUBJECT' => $message['SUBJECT'] ?? Loc::getMessage('MAIL_MESSAGE_ACTIONS_SUBJECT_PLACEHOLDER'),
				'MAIL_FROM' => htmlspecialchars_decode($message['FIELD_FROM'] ?? '', ENT_QUOTES),
				'MAIL_DATE' => ($message['INTERNALDATE'] ?? $message['FIELD_DATE'] ?? null)?->getTimestamp() ?? '',
			]),
	),
	'CRM_ACTIVITY' => array(
